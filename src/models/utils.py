...
def evaluate_model(model, X, y, seed):
    ...
    scores = cross_val_score(
        model, X, y, scoring=make_scorer(f1_score), cv=cv, n_jobs=-1
    )
    return scores
...
def calculate_precision_recall_curve(model, trainX, trainy, testX, testy, alias):
    # metrics
    md_precision_train, md_recall_train, md_f1_train, md_auc_train = _metrics(
        model, trainX, trainy
    )
    md_precision_test, md_recall_test, md_f1_test, md_auc_test = _metrics(
        model, testX, testy
    )
...
def make_confusion_matrix(y_true, y_pred, alias, labels=None):
    ...
    fig = ff.create_annotated_heatmap(
        z, x=x, y=y, annotation_text=z_text, colorscale="RdBu"
    )

    # add title
    graph_title = str(alias).replace("_", " ")
    fig.update_layout(
        title_text=f"<i><b>Confusion matrix {graph_title}</b></i>",
...
def compute_rfecv(X, y, estimator, min_features):
    ...
    fig = px.line(
        y=mean_test_f1_score,
        x=range(
            min_features_to_select, len(mean_test_f1_score) + min_features_to_select
        ),
        labels={
    ...
    fig.add_vline(
        x=n_features_selected, line_width=3, line_dash="dash", line_color="#011C5D"
    )
    fig.add_vrect(
        x0=30, x1=n_features_in_rfecv, line_width=0, fillcolor="#D38F00", opacity=0.2
    )
...
def compute_boruta(X_train, y_train, estimator, n_features) -> list:
    ...
    boruta_selector = BorutaPy(
        estimator, n_estimators=n_features, verbose=0, random_state=set_seed
    )
    # Entraîner boruta
...
def feature_selection(fs_method, Xtrain, ytrain, estimator) -> list:
    ...
    if fs_method == "kbest":
        ...
        features = feature_selection_kbest(Xtrain, ytrain, nb_features=50)
